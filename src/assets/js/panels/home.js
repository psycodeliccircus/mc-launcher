...
class Home {
    ...
    async initNews() {
        ...
        if (this.news) {
                ...
                blockNews.innerHTML = `
                    <div class="news-header">
                        <div class="header-text">
                            <div class="title">Nenhuma notícia disponível no momento.</div>
                        </div>
                    </div>
                    <div class="news-content">
                        <div class="bbWrapper">
                            <p>Você pode acompanhar todas as novidades relacionadas ao servidor aqui.</p>
                        </div>
                    </div>`
            ...
            blockNews.innerHTML = `
                ...
                <div class="news-content">
                    <div class="bbWrapper">
                        <p>Não foi possível contatar o servidor de notícias.</br>Verifique sua configuração.</p>
                    </div>
                </div>`
            // news.appendChild(blockNews);
        }
    ...
    async initLaunch() {
        document.querySelector('.play-btn').addEventListener('click', async () => {
            ...
            launch.on('check', (progress, size) => {
                progressBar.style.display = "block"
                document.querySelector(".text-download").innerHTML = `Verificação ${((progress / size) * 100).toFixed(0)}%`
                progressBar.value = progress;
                progressBar.max = size;
            });
            ...
            launch.on('patch', patch => {
                console.log(patch);
                info.innerHTML = `Correção em andamento...`
            });
            ...
            launch.on('data', (e) => {
                ...
                info.innerHTML = `Inicialização em andamento...`
                console.log(e);
            })
    ...
    async initStatusServer() {
        ...
        if (!serverPing.error) {
            ...
            serverMs.innerHTML = `<span class="green">On-line</span> - ${serverPing.ms}ms`;
            online.classList.toggle("off");
            playersConnected.textContent = serverPing.playersConnect;
        } else if (serverPing.error) {
            nameServer.textContent = 'Servidor indisponível';
            serverMs.innerHTML = `<span class="red">off-line</span>`;
        }
    }
